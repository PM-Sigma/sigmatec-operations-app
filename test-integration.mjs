...
const PROPAGATION = [
  ...
  // a 🔥 burn was marked → burnsData.ts invalidates its own TanStack cache, which both the
  // home card (Burns.tsx) and the table (BurnsPage.tsx) read through useBurns/useBurnGenerators.
  // Round 5 G-L2 moved the emit+listen pair here from the legacy file; G-U4 retired that file,
  // so burnsData.ts is now both the sole emitter and the sole consumer.
  ['burns-changed', ['app/src/lib/burnsData.ts'], ['app/src/lib/burnsData.ts']],
