...
`PmToday`, `Burns` (🔥 full table). Pure logic lives in `app/src/lib/*.ts` (vitest); the EMS door is
`app/src/lib/ems/{types,gateway,adapters/rest}.ts` — `app/src` has zero direct EMS calls left, all 13
typed operations go through `EmsGateway`, published to legacy as `sigma.ems`/`sigma.emsWrite`.
