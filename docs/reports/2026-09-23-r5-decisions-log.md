...
| 24 | 24.9 | DOC | The documentation coverage checks are warnings only until DOC-1 (`DOCS_STRICT=1`); the check that retired paths don't come back stays strict | Otherwise every merge that adds a file breaks all the packages | Set DOCS_STRICT=1 |
| 25 | 24.9 | Process | Paused at a clean point: every agent committed; nothing half-done in production; the hourly report is stopped | עידן's request | Resume: `docs/superpowers/r5-PAUSE-STATE.md` |
