🟡 IN PROGRESS (2026-09-17→) — **סיגמה 2.00 redesign**: spec `docs/superpowers/specs/2026-09-17-kibbutz-cards-redesign-design.md`, plan `docs/superpowers/plans/2026-09-17-kibbutz-cards-redesign.md`, branch `feat/kibbutz-cards-redesign` (worktree `SigmatecOps-wt-cards`). Tasks 0, 1, 1b done; 2 in fix round. **Task 17 (📈 שימוש — usage analytics + weekly narrative digest) built, tested and deployed on the branch** (`db/usage_events.sql` applied; `push-send` **v11** carries mode `usageDigest`, hardened in review fix round 1 — cron-key/EMS auth, no sentences in the response, and no user-typed text in analytics) — **two prod steps open: set the `CRON_SECRET` Edge-Function secret, then run `db/cron_usage_weekly.sql` (`push-usage-hourly` + the attendance job re-scheduled with the header) after the merge**, until then no digest is pushed. Task 6 (📣 feedback box + `transcribe`) built, tested, hardened (fix round 1) and deployed on the branch — **one manual step open: the Whisper server runbook `docs/whisper-server.md` (עידן), until then transcription runs on Groq**. Deferred to **Task 18 (ops)**: a pg_cron job for the `feedback-audio` 7-day retention sweep (the statement is in `db/feedback.sql`; today it is manual). Companion specs: unified inventory (17.9), company process (18.9, draft).
