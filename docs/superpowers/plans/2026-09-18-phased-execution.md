...
| **P4 — AI & meetings** | 16 (day-log AI), then the company-process tasks planned 18.9 22:20 (`2026-09-17-kibbutz-cards-redesign.md` → **## P4 — Company process**, from spec `2026-09-18-company-process-design.md`): **24** meeting presenter mode + live quick-note (Opus, L) · **25** review-as-editor (Opus, L) · **26** internal tasks 🔒 (Sonnet, M) · **27** onboarding template (Sonnet, M) · **28** health v1 draft (Opus, M) · **29** Clockify per kibbutz (Opus, L) · **30** dev meeting mode + sprint prep (Opus, M). 24–30 are independent of one another; the order above is by value, so a closed budget window drops from the end. **Gmail intake-lite is excluded** — not approved (spec §8b). Handoff items: `CLOCKIFY_API_KEY`, `CLOCKIFY_WORKSPACE_ID`, Drive folder ids, health thresholds (Tue 22.9). | PM/CEO loop | P2, P3 |
