...
- **QA gates (עידן 18.9, binding from Task 22 on):** every task runs `npm run qa` (Gitleaks 0 findings · Semgrep 0 · Playwright desktop+mobile light/dark green · Lighthouse thresholds · ZAP baseline 0 High/Medium · `npm test`) and attaches `qa/reports/<date>-task-N.md`; the reviewer rejects a task without it. No email sending from the app (G5 ruling).
- **Visual parity gate (spec §6, עידן 18.9):** every UI task attaches side-by-side screenshots (mockup vs app, 390/1440, light/dark) and a performance trace of its heaviest interaction; layout/spacing/type/motion must match the mockup unless the spec names the deviation.
...
- [ ] 5b. **Visit-summary + attendance re-skin (spec §6):** restyle `#tab-visit`/`visitQuickModal` and the attendance page with tokens + Assistant; day-type buttons as a ToggleGroup-styled row, **duration = quick hour chips (1/2/3/4/6) + a ☀️ יום שלם chip that maps to the existing `visitWorkday` flag and disables hours, plus the existing 0.25-step hours input for exact values** (עידן, mockup comment 17.9: keep the full-day option per the existing spec), products as a clean checkbox list with quantity steppers, **the free-text summary split into two fields (עידן, mockup comment 17.9): "מה עשיתי בביקור" (existing `summary`) and "מה נשאר לי פתוח" (new `visits.open_items text` column, `db/visits_open_items.sql`); the briefing's ביקור קודם block shows open items under a highlighted "נשאר פתוח" header, and the visits PDF/Excel get an extra column;** cert status chip restyled, monthly attendance table readable on 390 px. Logic untouched — `test-visit-cert-gate.mjs`, `test-attendance-push.mjs`, `test-attendance-toggle.mjs` must pass unchanged.
- [ ] 5b2. **Visit drafts (spec §5.1c):** `db/visit_drafts.sql` (table + RLS + 14-day sweep note), legacy `09-visits.js`: `visitDraftSave()` debounced 800 ms on input/change + on `switchTab`/`pagehide`/`visibilitychange` (payload = the form's current values incl. products, workday/duration, did/open, contact), `visitDraftRestore(id)`, `visitDraftDiscard`, delete on `saveVisit` success; bridge `sigma.visitDraftFor(kibbutz, person, date)`; React: draft chip on `KibbutzCard`/briefing, "המשך טיוטה" prompt in `openVisitQuick(name)`; 🚚 path: no form/draft → toast "נדרש קודם סיכום ביקור — פותח את הטופס" then open form; tests: pure `draftState`, debounce with fake timers (legacy node test with a fake `setTimeout`), restore round-trip golden.
...
### Task 22: QA gates infrastructure (Gitleaks · Semgrep · Playwright · Lighthouse · ZAP) + Playwright backfill

**Agent:** Opus. Runs FIRST (P0). Files: `qa/README.md`, `qa/gitleaks/.gitleaks.toml` + allowlist for the public Supabase anon key and VAPID public key (they are public by design — document why), `qa/semgrep/config.yml` (rulesets p/default, p/owasp-top-ten, p/javascript, p/typescript; offline cache under `qa/semgrep/.cache`), `qa/playwright/playwright.config.ts` (projects: desktop-1440 light/dark, mobile-390 light/dark, RTL; baseURL = local static server started by the config on port 8123 serving the repo root; `?login=0&sb=0`), `qa/playwright/tests/*.spec.ts` backfill: home cards (sections, region labels, filters, quick actions), kibbutz sheet (both modes, validation), meeting notes (render, collapse), EMS tasks (clamp toggle), nav + ⋯ sheet + user menu + settings + landing per role, Ctrl+K, visit form (drafts autosave/restore, cert relabel, split fields), feedback box (states; no mic), usage page (עידן only), viewer shell — each asserting no console errors and taking screenshots to `qa/playwright/shots/`; `qa/lighthouse/lighthouserc.json` (mobile preset, thresholds perf 85 / a11y 95 / bp 95) + runner; `qa/zap/baseline.sh|ps1` (Docker `zaproxy/zap-stable` `zap-baseline.py` against http://host.docker.internal:8123; if Docker is absent, document the ZAP installer path and a manual run); `scripts/qa.mjs` = `npm run qa` orchestrator (runs all six, writes `qa/reports/<date>-<label>.md`, non-zero on any failure), `.githooks/pre-commit` (gitleaks protect --staged) + `git config core.hooksPath .githooks`. Installation must be local and offline-capable after first install: gitleaks via `winget install gitleaks` or the GitHub release binary into `qa/bin/`; semgrep via `pip install semgrep` (if the Windows wheel is unavailable, use `pipx`/WSL or Docker `semgrep/semgrep` — record which); Playwright via npm + `npx playwright install chromium`; Lighthouse via `npm i -D @lhci/cli`. Steps: install → configs → backfill specs → run `npm run qa` on the current tree → fix what fails (or document accepted findings with justification in `qa/reports/`) → commit. Global Constraints gain: "Definition of Done = `npm run qa` green + reviewer sees `qa/reports/<date>-task-N.md`".

### Task 21: Session & access hardening (spec §7n)

**Agent:** Opus. Depends on 0. Runs right after Task 4. Files: `supabase/functions/ems-auth/index.ts` (exp 60→180 min), `js/src/15-login-gate.js` + `js/src/12-reports.js` (re-mint timer every 50 min + on visibilitychange; global 401 interceptor around `emsApi` and the supabase-js client → `sigmaBus` `session-expired`), `app/src/components/ReLoginSheet.tsx` (mounted once, full-screen, preserves page/scroll/draft; listens to `session-expired`), `app/src/lib/session.ts` (`gateState()`, `useEmsGate()` used by every island: render `<LoginRequired/>` when not connected), RLS tightening SQL `db/rls_authenticated_only.sql` for kibbutzim/notes/visits/visit_drafts/feedback/usage_events (verify anon gets 0 rows; the login flow still works), probe script `scripts/ems-auth-probe.mjs` documenting EMS TTL/refresh findings in `docs/ems-session.md`, and if no refresh exists a GitHub card via the `github` function (parent הרשאות). Disable `?login=0` on the production origin (allow only on localhost / raw.githack preview). Tests per spec §7n. Steps: tests → implement → suite → build → deploy `ems-auth` (diff vs origin/main) → apply RLS migration → smoke: expire the token artificially (`_sbTokenExp = 0`) on the inventory page → one sheet → login → same page + draft intact → commit.
...
**Interfaces:**
- SQL: `feedback(id uuid pk, author text, kind text check (kind in ('idea','bug','complaint')), text text not null, audio_path text, status text default 'new' check (status in ('new','seen','done')), github_issue int, created_at timestamptz default now())`; inbox action for `bug` rows → `github` edge fn mode `createIssue` (add if missing: title/body/labels → returns number) → PATCH `github_issue` + RLS; Storage bucket `feedback-audio` (private; upload via authenticated; admins read via signed URL).
...
- **Step 0 (עידן 18.9 — free-first prototype):** on עידן's server (already has Whisper) run `docker run -d -p 8000:8000 -v whisper-cache:/root/.cache/huggingface fedirz/faster-whisper-server:latest-cpu` (or the `-cuda` tag), set `WHISPER__MODEL=ivrit-ai/whisper-large-v3-turbo-ct2`; expose with `cloudflared tunnel --url http://localhost:8000` (quick tunnel for the prototype; named tunnel + Access service token for production); test `curl -F file=@note.ogg -F model=ivrit-ai/whisper-large-v3-turbo-ct2 -F language=he https://<tunnel>/v1/audio/transcriptions`. Record latency and Hebrew accuracy on 3 real voice notes vs base `large-v3`. Document the server runbook in `docs/whisper-server.md` (start/stop, model swap, token rotation).
- `transcribe/index.ts`: verify caller JWT (anon ok — payload is just a storage path in our bucket), download from Storage with service role, POST multipart **first to `SELF_WHISPER_URL/v1/audio/transcriptions`** (bearer `SELF_WHISPER_TOKEN`, `model=ivrit-ai/whisper-large-v3-turbo-ct2`, `language=he`, 25 s timeout), and only on failure/timeout (and if `GROQ_FALLBACK=on`) to `https://api.groq.com/openai/v1/audio/transcriptions` (`model=whisper-large-v3`, `language=he`) with `GROQ_API_KEY`; return `{text, engine:'self'|'groq', ms}`; on total failure `{error}` 502. Write a row to `transcribe_log(engine, ms, ok, audio_sec)` for the health panel. Max 25 MB.
...
### Task 11: Dev page redesign — 4 full columns + minimized rail + tree view + flow strip

**Agent:** Opus. Depends on 0, 4. **Spec §7d.** Files: `js/src/18-dev-tasks.js` (board layout, rail, tree toggle, flow strip; keep write paths), `css/app.css` `.dev-*` (stage tokens `--stage-*` + dark variants), `index.html` `#dev-view` markup, `test-devboard.mjs` (extend: `devBoardLayout` golden, `devTree` golden incl. hideDone root pruning, flow-strip proportions sum 100; existing devStage tests untouched). Decision: the dev page stays legacy vanilla (it is a single self-contained module with working DnD); only styling tokens come from Task 0. Steps: tests → implement (`devBoardLayout(tasks, expanded)` → `{full:[{key,tasks}], rail:[{key,count}]}` with `FULL_KEYS=['ready','prog','review','backlog']`; `devTree(tasks,{hideDone,assignee,stage,q})`; `renderDevTree()`; `renderFlowStrip()`; mobile snap-scroll; remembered view per device `localStorage['dev_view']`) → suite → build → smoke desktop 1440 + phone 390, both themes, DnD still works, 🚀 עלתה גרסה still works → commit.

### Task 12: Attendance redesign + holidays

**Agent:** Opus. Depends on 4 (5b re-skin), 5. **Spec §7e.** Files: Create `db/company_holidays.sql` (+ seed rows), `db/holidays_seed.mjs` (Hebcal fetch → SQL; chol hamoed Sukkot 2026 → `company_closure`), `app/src/islands/Attendance.tsx` (phone-dense layout: today card, KPIs, month grid, missing chips, day sheet; desktop split view), `app/src/lib/attendance.ts` (`monthGrid(year,month,rows,holidays)`, `missingDays(rows,holidays,today)`, `kpis(rows)`) + vitest goldens (September 2026 fixture per spec), `app/src/islands/Holidays.tsx` (admin toggle list under ⋯ עוד); Modify `js/src/04-attendance-daily.js` / `22-push.js` (`attMissingDays` reads holidays from the shared `SHEET_DATA.holidays` loaded once per session via supabase-js), `supabase/functions/push-send/index.ts` (`priorMissing` + evening gate skip `company_holidays where required=false`), `test-attendance-push.mjs` (extend with holiday skip). Existing report builders (monthly PDF/Excel) get a 🕎 marker column for filled holiday rows. Steps: tests → implement → suite → build → apply migration + seed → deploy `push-send` (diff vs origin/main first) → smoke as ניתאי on 390 px: 13.9/21.9 show חג, 27.9–2.10 חול המועד, missing list excludes them, entering 21.9 works → commit.
...
### Task 14: calendar **list view** (replaces the standalone משימות page — §7m R1) + retirements R2–R8

**Scope change (עידן 18.9):** no separate page. Add the third view **רשימה** to the calendar island (my open tasks grouped by kibbutz, overdue first, row actions 📅 שבץ / ✓ סיים / 📍 בריפינג / שתף). Then retire per §7m: legacy EMS page (keep task detail/create/edit modals + their bridge functions), משימות page, עובדים page, `kibbutz-stats.html`, the home-page "משימות חברה" block+modal (data → internal tasks with `kibbutz=null` via a one-shot migration of `companyTasks` rows), the "משימות באחריותי" report (keep a שתף action). **Before removing anything, read `docs/reports/2026-09-18-retirement-coverage.md` and implement every row marked MOVE in the same task; rows marked KEEP stay; rows marked GAP block the retirement of that function until the controller rules.**

(original text follows)
### Task 14 (original): "משימות" page island
...
- **EmsGateway (spec §7o):** introduce `app/src/lib/ems/{gateway.ts,types.ts,adapters/rest.ts}` + bridge `sigma.ems.*`; move every direct `emsApi`/`fetch` EMS call (legacy + React + edge functions' Deno copy) behind it with golden request/response mapping tests; add the contract grep (no `emsApi(`/EMS `fetch(` outside the adapter); `capabilities()` wired to button visibility. Behavior byte-identical.
...
### Task 19: System Requirements & Architecture Document (SRS) for an external systems reviewer (עידן 18.9)

**Agent:** Opus (writing) — runs when modeling is complete (after the company-process spec is planned), BEFORE Task 7 release. Output: `docs/SRS-סיגמה-2.0.md` + `docs/SRS-סיגמה-2.0.docx` (via the `sigmatec-nohal`/docx skill, Hebrew RTL, company template) and a PDF. Audience: a systems analyst/reviewer who must judge "אפשרי, הגיוני, טוב".
Contents (IEEE-830-style, but readable): 1. Purpose, scope, roles & personas (עידן, עמיחי, אביאם, ניתאי, מתניה, צופה, the kibbutz as an external actor). 2. Business processes as swimlane diagrams (Sunday meeting loop, field day loop, order → stock → visit → cert, onboarding, hours → report) — Mermaid rendered to images. 3. Functional requirements numbered FR-xx per module (cards, meeting notes, field flow, inventory, alerts, calendar, tasks, attendance/holidays, dev page, settings/gaps, day-log AI, transcription, usage analytics, meeting mode, internal tasks, PM inbox-lite, onboarding, health, work sessions/Clockify, feedback), each with acceptance criteria and the test that proves it (from the plan). 4. Non-functional: RTL/Hebrew, performance budgets (boot ≤ 90 kB gzip, ≤ 250 ms motion), offline-first behavior, security (RLS, EMS-gated JWT, secrets only in edge functions, no PII in analytics), privacy of recordings, availability (GitHub Pages + Supabase), backup/rollback (append-only ledgers, archive-not-delete). 5. Architecture: C4 context/container diagrams (PWA + React islands ↔ Supabase (Postgres/RLS/Storage/Edge/pg_cron) ↔ EMS API ↔ Google (Calendar/Drive/Gmail/Meet) ↔ GitHub Projects ↔ Whisper server ↔ Gemini/Groq ↔ Clockify), data model ERD of every table with owners, the event bus contract (`docs/integration-map.md`), the bridge contract, build/deploy pipeline, environments and secrets inventory. 6. Data flows per feature with failure modes and fallbacks (push undelivered → in-app banner; Whisper down → Groq; EMS offline → queue). 7. Risks & mitigations, open decisions (from every spec's open-items table), phased delivery (2.00 / 2.01 / 2.02) with what is already built. 8. Traceability matrix: עידן's request → spec section → task → test. 9. Glossary (Hebrew/English). Reviewer checklist appendix: the questions the document should let a reviewer answer in one sitting.
Method: assemble from the three specs, the plan, the ledger rulings, `integration-map.md`, and the UX/adoption reports; no new requirements invented — gaps become explicit "open" items. Self-check: every FR traces to a task; every table in `db/*.sql` appears in the ERD.

### Task 6b: Wire the live Whisper server (fast + refine) into `transcribe` and the voice component

**Agent:** Sonnet. Depends on 6. **Spec §7i "Whisper server — live".** Files: `supabase/functions/transcribe/index.ts` + `chain.ts` (self URL constant `https://idanhomepc.tail9e880d.ts.net`, `language=he` always, retry once on network/5xx after 2 s, never on 4xx, then Groq; pass through `job_id`/`refine_eta_seconds`; new `GET`-style mode `{job_id}` → proxy the refine poll with the bearer token; cache by job_id in `transcribe_log`), `app/src/lib/speech.ts` + `VoiceInput` (after fast text: poll every `min(seconds_remaining, 5)` s while the field is open and untouched; on refined text → replace + "עודכן" chip + undo; if edited/sent → discard; stop polling on unmount), `docs/whisper-server.md` (replace the Docker section with the actual server facts + token rotation), tests: chain (self ok / self 5xx→retry→Groq / self 4xx→Groq no retry / timeout), pure `refineMerge(state, refined)` (untouched → replace; edited → discard; sent → discard). Secrets: `SELF_WHISPER_TOKEN` set by עידן (the agent must not print it). Steps: tests → implement → deploy `transcribe` (diff vs origin/main) → smoke with a Hebrew clip: fast text < 25 s, refine arrives, "עודכן" shows → commit.
