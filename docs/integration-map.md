...
| 🔒 פנימי chip — **hidden** | — | `internal_tasks` exists (`db/internal_tasks.sql`, Task 14) but the client only READS it; Task 26 ships the write path. A chip that silently does nothing mid-meeting is worse than no chip, so `INTERNAL_TASKS_WRITABLE` in `islands/Presenter.tsx` is the one flag Task 26 flips. |
