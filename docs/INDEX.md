...
## 🚦 Current state — last: 2026-06-25 (**·86 LIVE on `main`**). **💻 Dev page (פיתוח) — sprint board LIVE:** default **status board** with 6 named columns (ממתין לפיתוח/ספרינט קרוב/בפיתוח עכשיו/בשלבי בדיקות/גמר פיתוח ממתין לגרסה/עלה לאוויר) + view toggle (סטטוס/נושא, topic tree kept); **multi-select → דחוף ל-Ready** + **🚀 עלתה גרסה** (Done→Committed) via the `github` fn **`mode:"setStatus"`** (`setProjectStatus`, EMS-gated — needs `GH_TOKEN` Projects-v2 **write** + a "Committed" status option, both done); **status-entry day-stamps** via Supabase **`dev_status_log`** (forward-tracking; `db/dev_status_log.sql` run); **offline ticket cache** (fetch once per connection); page visible to **עידן+עמיחי+מתניה+אליה**. *(`feat/dev-sprint-board` merged → dev → main.)* — AI order parsing **verified live end-to-end** (Gemini via `parse-order`, EMS-gated; Gemini→Groq chain; offline matcher fallback). Customer accessories: Landis(incl E570)→SIM, non-Landis meter→controller; controller & power-supply TYPE via a **conversational modal**; ambiguous "סאטק"→asks model; non-catalog items prompt add-to-catalog-or-drop at save; every text order feeds the learning loop; **parse-source badge** (Gemini/Groq/Offline) on the items label; updater's stock 📦 per item; "תאריך הזמנה" (editable; created_at=entry) + delivery date shown. **Deferred:** customer delivery auto-stamp on EMS task-closure (needs `ems_task_id` col — `db/orders_ems_task_id.sql`); order activities in "פעילות היום". **TODO:** add `Carlo Gavazzi E341` to the מוצרים catalog (or let the add-to-catalog prompt do it).
