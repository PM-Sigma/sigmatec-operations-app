...
|------|------------------------|------------------|
| 🧑‍💻 **DEV-PAGE** (פיתוח) | `app/src/islands/DevBoard.tsx` + `app/src/islands/dev/*` (React, D-U1/D-U3 — `18-dev-tasks.js` retired) · `app/src/islands/DevPresenter.tsx` (▶ ישיבת פיתוח) · `app/src/lib/devBoard.ts` + `devMeeting.ts` + `devMarks.ts` + `devFlow.ts` + `devStatusLog.ts` + `sprintPrep.ts` · `supabase/functions/github/` · `db/dev_status_log.sql` | **תחומים view** — SectionBlock per GitHub-parent domain ("ללא אפיון" last) → priority tier, "חדש השבוע" first · **שלבים view** — the four full stages + a rail for the rest · a flow strip (StackedBar, tap a slice = filter) · a filters sheet · a card sheet (priority chips + "העברה לשלב", gated to עידן/עמיחי/מתניה — `canWriteGithub`, the same roster `github`'s own `gate.js` enforces server-side) · **▶ ישיבת פיתוח** walks the same domain grouping with LOCAL marks (לספרינט/לבירור/לדחות, never written to GitHub) and an end-of-meeting summary · **day-stamps** (`dev_status_log`) · access for **מתניה + אליה** (+ עידן/עמיחי) |
...
Round 3 added the running-timer sheet (pause · retime · 2 h auto-stop) and the ⏱ שעות מול לקוחות page.
Three migrations wait for a human (`db/internal_tasks_fields.sql`, `db/meter_burns_ems_task.sql`,
`db/work_sessions_log.sql` — required for edits on the hours page).
Full detail: `docs/CHANGELOG.md` [2.08]. **Next:** merge to `dev`, QA session (סשן B — בקר Opus)
runs `npm run qa` + phone scenarios (every dialog closes, background→foreground); then D13 with עידן.
