...
commit; `main` stays on maintenance mode (`e398947`) until עידן completes the parked production
steps below and the controller fast-forwards `dev`→`main` (see `docs/HANDOFF-עידן.md`).

**Why:** the 1.x app grew page-by-page around EMS tasks; 2.00 rebuilds the home screen around the
**קיבוץ card** as the unit of work — meetings, EMS tasks, visits, health and onboarding all attach to
one card — and gives the field team (אביאם/ניתאי) a real arrival→briefing→visit-summary flow instead
of manual note-taking. New surfaces are React islands (`app/src`, Vite+TS+Tailwind+shadcn, built to
`ui/sigma.js|css`); legacy vanilla (`js/src/*.js`→`js/app.js`) stays for everything not rebuilt.

**By part** (full detail in `.superpowers/sdd/2026-09-17-kibbutz-cards-redesign/progress.md` and the
per-task `task-N-report.md` files in the same folder):
- **Cards + kibbutzim table (T1/T1b):** `kibbutzim` table replaces the static card grid; region
  grouping, alphabetical order, energy-gate warnings (`applyCardSiteWarnings`), 🔥 burns chip.
- **Meeting bullets + full EMS tasks on the card (T2/T3):** meeting notes parsed and merged onto
  cards with an alias-aware canonical-name merge key (renamed kibbutzim keep their EMS link).
- **Field arrival/briefing + visit-summary push (T5):** 2-hour post-visit push, delivery-cert gate
  (rules 1–4) kept byte-identical, fast visit path.
- **Feedback box + Hebrew voice (T6/T6b):** `FeedbackInbox` island, Groq Whisper transcription via a
  new `transcribe` edge fn (self→groq chain), GitHub issue creation (`github` fn `createIssue`).
- **Brand/dark-mode refresh + nav retirement (T4/T11/T14):** `--brand-1/-2/-grad` tokens, dark mode
  via `data-theme`, dev-page redesigned (4 columns + rail + tree view), retired pages
  (משימות/EMS/עובדים/stats.html) folded into רשימה (calendar's 3rd view) and the home command-bar.
- **Unified calendar + presenter mode + meeting review (T13/T24/T25):** month/week/list views, route
  order via drag, EMS scheduler, absences, ▶ presenter overlay for live meetings with quick-notes
  that create EMS tasks immediately, post-meeting review editor feeding `import_meeting_notes`.
- **Day-log + internal tasks + onboarding + health v1 (T14/T16/T26/T27/T28):** 📝 יומן היום (Gemini→Groq
  parse chain, nothing server-trusted — kibbutz/product/task all re-validated against the live EMS
  catalog before it can touch stock), internal tasks, onboarding templates + progress strip, health
  v1 draft scorers (thresholds open, decision pending 22.9).
- **Usage analytics + gaps + settings (T15/T17):** 📈 שימוש weekly digest, 📋 הפערים שלי pure gap
  detector, per-person EOD hour + gap-reminder push mode.
- **EmsGateway + integration hardening + RLS (T18a/T18b):** one typed door to EMS
  (`app/src/lib/ems/{types,gateway,adapters/rest}.ts`, 13 ops) replacing all direct calls from
  `app/src`; `docs/integration-map.md` + `test-integration.mjs` (123 contracts) generated by
  `node build.mjs`; three real defects found and fixed (missing `#pushlog-view`, ignored
  `holidays-loaded`, lazy-island open race — the real cause of the daylog flake); two RLS lockdown
  migrations closing anonymous read on `delivery_certs`/`field_checkins` and on the corrections table.
- **🔥 צריבות temporary project (T23):** merged from `feat/meter-burns-rel` (1.71) — this merge also
  brought main's 1.67 (site consolidation, attendance hub, login self-heal) onto this branch for the
  first time; 17 conflicted files, 3 silent git mis-merges found and repaired, reviewer confirmed no
  4th. `BURNS_PROJECT_ACTIVE` flag hides the whole feature if needed.
- **Clockify + dev-presenter (T29/T30):** ▶/■ work-session timer on kibbutz cards, sprint-prep +
  dev-presenter island reusing the existing `github` fn read.
- **SRS (T19):** `docs/SRS-סיגמה-2.0.md`/`.docx`/`.pdf`, 86 functional requirements, 13 diagrams.

**Manual smoke: PENDING (עידן)** — logged-in smoke against production data is blocked on the parked
migrations/deploys/secrets in `docs/HANDOFF-עידן.md`; must be done and recorded here before the
`dev`→`main` fast-forward.

### [2.00 detail] Stale-while-revalidate, משיכה לרענון ורענון מטמון EMS מהמשרד (Task 20)
...
### [2.00 detail] 🔥 צריבות נכנסו ל-2.00 כפרויקט זמני (Task 23)
...
### [2.00 detail] 📈 שימוש: usage analytics + weekly narrative digest (עידן בלבד)
...
### [2.00 detail] 📣 תיבת רעיונות / באגים / תלונות + תמלול עברית
