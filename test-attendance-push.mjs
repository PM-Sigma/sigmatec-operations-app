...
function loadModule() {
  const fn = new Function('window', 'document', 'localStorage', 'navigator', 'fetch', 'setTimeout',
    'SB_URL', 'SB_ANON', 'getCurrentUser', 'isViewer', 'isIdan', 'attPerson', 'confirm', 'alert',
    src + '\nreturn { attMissingDays, attReminderText, PUSH_USERS, pushB64ToU8 };');
  return fn({}, { getElementById: () => null, createElement: () => ({ style: {} }), body: { appendChild() {} } },
    { getItem: () => null, setItem() {} }, { userAgent: 'test' }, async () => ({ ok: true }), () => {},
    'http://x', 'anon', () => '', () => false, () => false, () => '', () => false, () => {});
}
const M = loadModule();
...
console.log('== reminder text + recipients ==');
...
check('recipient allowlist = field/approval trio', () => {
  assert.deepStrictEqual(M.PUSH_USERS, ['אביאם', 'ניתאי', 'עמיחי']);
});
check('VAPID base64url decoder yields 65-byte P-256 point', () => {
  const u8 = M.pushB64ToU8('BGlLTZ-fWd8PZN35Y0gEm8WlPQfuZfIZuv_owazHbuvmN-LTMDASgn8F-dqplIpD4cqSkw9axAjEPAc7zui0XgI');
  assert.strictEqual(u8.length, 65);
  assert.strictEqual(u8[0], 4);   // uncompressed EC point marker
});
