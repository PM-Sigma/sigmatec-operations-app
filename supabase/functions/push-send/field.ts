...
export function openNudges(i: {
  ...
  for (const c of i.checkins || []) {
    ...
    out.push({
      ...
      hasDraft,
      text: hasDraft
        ? `התחלת לכתוב על ${c.kibbutz}. עוד דקה והסיכום נשלח.`
        : `${c.kibbutz} · עוד לא סיכמת את הביקור`,
    });
  }
  return out;
}
...
export const VISIT_NUDGES: Nudge[] = [
  { t: 'עוד לא סיכמת את הביקור', b: 'מה נעשה ב{kibbutz} ומה נשאר פתוח? שתי דקות עכשיו חוסכות טלפונים בסוף החודש.' },
  { t: 'סוגרים עכשיו, נחים אחר כך', b: 'כמה שורות על {kibbutz} עכשיו, ובסוף החודש אין מה לשחזר.' },
  { t: 'קפה בנחת ב-30 לחודש', b: 'סיכום של {kibbutz} עכשיו, ובסוף החודש לא צריך לרדוף אחרי מה היה שם.' },
  { t: 'מה שכתוב נשאר', b: 'מה שבראש נשכח עד הישיבה. שתי שורות על {kibbutz}, ויש לך מה להגיד בה.' },
  { t: 'מתחילים בשתי שורות', b: 'תכתוב שתי שורות על {kibbutz}. בדרך כלל השאר מגיע לבד.' },
  { t: 'פחות הפתעות בחיוב', b: 'סיכום קצר של {kibbutz} עכשיו חוסך טלפונים והפתעות בחיוב של החודש.' },
  { t: 'עוד וי אחד להיום', b: 'כמה דקות על {kibbutz}, והביקור סגור.' },
  { t: 'כמה דקות עכשיו, שעה בסוף החודש', b: 'הסיכום של {kibbutz} לוקח עכשיו כמה דקות. בסוף החודש הוא ייקח שעה.' },
  { t: 'הקיבוצים לא ינהלו אותך', b: 'סיכום של {kibbutz} באותו יום, וכל שאלה מהמשרד כבר קיבלה תשובה.' },
  { t: 'בלי לחץ של סוף חודש', b: 'סוגרים את {kibbutz} היום, ובסוף החודש אין מה להשלים.' },
  { t: 'צעד קטן', b: 'הסיכום של {kibbutz} הוא מה שהמשרד והלקוח רואים מהביקור שלך.' },
  { t: 'שלוק מים ושתי דקות', b: 'זה כל מה שצריך כדי לסגור את הפינה של {kibbutz}.' },
  { t: 'שקט בראש בערב', b: 'סיכום של {kibbutz} שנכתב עכשיו לא יחכה לך בלילה.' },
  { t: 'שתי דקות עכשיו, או רבע שעה אחר כך', b: 'לשחזר את {kibbutz} בסוף החודש לוקח פי ארבעה.' },
  { t: 'ביקור מתועד באותו יום', b: 'ככה בונים שם של איש שטח מסודר. {kibbutz} מחכה לשתי שורות.' },
  { t: 'יאללה, לגמור עם זה', b: 'מוזיקה ברקע, שתי שורות על {kibbutz}, ואתה משוחרר.' },
  { t: 'נשאר דבר אחד קטן', b: 'שתי דקות על {kibbutz}, והיום סגור.' },
  { t: 'הרשימה שלך כמעט נקייה', b: 'נשאר רק הסיכום של {kibbutz}. אחריו אפשר להתפנות.' },
  { t: 'סדר עושה שקט', b: 'כל ביקור שנסגר הוא דבר אחד פחות בראש. הסיכום של {kibbutz} הבא בתור.' },
];

/** Adoption report §3.5, 📝 יומן היום. Their mode is Task 16; the words are pinned here. */
export const DAYLOG_NUDGES: Nudge[] = [
  { t: 'שתי דקות והיום סגור', b: 'ספר לטלפון מה היה היום. הבינה המלאכותית מסדרת, ואתה רק מאשר.' },
  { t: 'תן לפה לעבוד במקום לאצבעות', b: 'שני משפטים בקול על מה שקרה היום, וזה במערכת.' },
  { t: 'סוף יום', b: 'דקה של דיבור מתעדת את כל היום. אחרי זה אפשר לנוח.' },
];
...
export const GAP_NUDGES: Nudge[] = [
  { t: 'כמה דברים קטנים מחכים לך', b: 'שתי דקות ברשימת הפערים שלך, והיא נקייה.' },
  { t: 'הרשימה שלך כמעט נקייה', b: 'נשארו {n} פריטים ברשימת הפערים. עוד קצת והיא ריקה.' },
  { t: 'סדר עושה שקט', b: 'מבט קצר ברשימת הפערים. כל פריט שנסגר שם הוא דבר אחד פחות בראש.' },
];
...
export const RECOUNT_NUDGES: Nudge[] = [
  { t: 'כמעט שם עם המלאי', b: 'נשאר להסביר בכמה מילים מה קרה בספירה, ואז היא מתועדת.' },
  { t: 'תודה על הספירה', b: 'ספירה מדויקת עכשיו חוסכת בלבול בסוף החודש.' },
  { t: 'מלאי מדויק, ראש שקט', b: 'הערה קצרה על הספירה של {product}, וסיימת.' },
  { t: 'ספירה מתועדת, הזמנה בלי הפתעות', b: 'ספירה שמתועדת היום חוסכת הפתעות בהזמנה הבאה. תודה על תשומת הלב.' },
];

/** The one draft-aware line (§5.1c): he already started, so the message is "finish", not "start". */
export const DRAFT_NUDGE: Nudge = {
  t: 'יש לך טיוטה פתוחה',
  b: 'התחלת לכתוב על {kibbutz}. עוד דקה והסיכום נשלח.',
};
