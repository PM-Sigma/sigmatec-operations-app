...
function buildPrompt(catalog: string[], examples: any[], text: string, orderType = "supplier"): string {
  ...
  const autoAddNote = orderType === "customer"
    ? "הזמנת לקוח — הוסף אוטומטית לפריטים (אלא אם הוזכרו כבר). הכלל: סים אחד לכל נקודת תקשורת.\n" +
      ...
      '• כל בקר (PURS למוני ASIC, וגם ROBUSTEL) — "סים פרטנר" ×1 לכל בקר\n' +
      '• אל תוסיף סים נפרד למוני SATEC עצמם — הסים שלהם נמצא בבקר ROBUSTEL\n' +
      '• משנ"ז פיזי (ללא המילה "מונה", כגון "משנ"ז 250") — ציוד חשמלי, לא מונה → אין סים\n' +
      "• אם המשתמש ציין סים/בקר במפורש — אל תכפיל, עדכן כמות לסכום הנדרש"
