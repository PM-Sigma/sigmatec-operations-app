...
const ALIASES: { when: string; product: string }[] = [
  { when: 'סאטק / Satec / "133" / EM133 — רק כשמוזכר סאטק או 133 מפורשות', product: "Satec EM133" },
  { when: "מונה/מונים תלת-פאזי או מונה כללי ללא מותג (ברירת המחדל הכללית למונים)", product: "מונה Landis+Gyr E360PP" },
  { when: "מונה לנדיס ללא וריאנט (ברירת מחדל)", product: "מונה Landis+Gyr E360PP" },
  { when: "מונה לנדיס ישיר לקו", product: "מונה Landis+Gyr E360PP" },
  { when: "מונה לנדיס חד פאזי", product: "מונה Landis+Gyr E360SP" },
  { when: 'מונה לנדיס משנה-זרם / מונה משנ"ז / מונה תלת-פאזי משנה זרם', product: "Landis+Gyr E360CT" },
  ...
  { when: 'EM133 יחד עם משנ"ז / CT חיצוני (לא "משני זרם"/"שנאי" — זה PM135)', product: 'מונה EM133 משנ"ז' },
  { when: "בקר 485 / RS485 / RS-485", product: "בקר 485" },
];
...
function buildPrompt(catalog: string[], examples: any[], text: string, orderType = "supplier"): string {
  ...
  const autoAddNote = orderType === "customer"
    ? "הזמנת לקוח — חלץ אך ורק את הפריטים שמוזכרים מפורשות בטקסט (מונים, משנ\"ז פיזי וכו'). " +
      "אל תוסיף אביזרים (סים / בקר / אנטנה / ספק כוח) — האפליקציה מוסיפה אותם אוטומטית אחרי הניתוח לפי כללי החברה."
  ...
  return [
    ...
    "מילון מונחים מחייב — כשמופיע הביטוי, מַפֶּה למוצר המדויק (גובר על ניחוש):\n" + glossary +
      "\nברירת מחדל למונים: בקשה גנרית למונים ('מונים', 'מונים תלת פאזי') ללא מותג → מונה Landis+Gyr E360PP. Satec EM133 רק כשמוזכר 'סאטק' או '133' מפורשות — לעולם לא כברירת מחדל. לבקשה גנרית אחת אל תחזיר כמה סוגי מונים — בחר אחד בלבד." +
      "\nשים לב: בקשה אחת יכולה לכלול גם מונים רגילים וגם מונה משנה-זרם — החזר את שניהם כשורות נפרדות (למשל '4 מונים תלת פזי' + 'מונה תלת פאזי משנה זרם' → 4× מונה Landis+Gyr E360PP + 1× Landis+Gyr E360CT)." +
      "\nמשנ\\\"ז: רק עם המילה 'מונה' ('מונה משנ\\\"ז' / 'מונה משנה-זרם') → מונה E360CT. 'משנז' עם מספר (250 / 400) או ללא המילה 'מונה' → המוצר הפיזי מהקטלוג ('משנ\\\"ז 250' / 'משנ\\\"ז 400'), לעולם לא מונה E360CT." +
      "\nEM133: מוזכר יחד עם 'משנ\\\"ז'/CT חיצוני (לא 'משני זרם'/'שנאי' — זה PM135) → מונה EM133 משנ\\\"ז. EM133 לבד (ישיר לקו) → Satec EM133 הרגיל.",
    autoAddNote,
    "קטלוג:\n" + cat,
