...
// ── inline the critical CSS (task 22b) ──────────────────────────────────────────────────────
// css/app.css + ui/sigma.css are loaded non-blocking (see the <head>), so what the first
// screen needs ships INSIDE the html. css/critical.css is the source of truth; this stamps its
// minified bytes between the two markers, so nobody has to keep a copy in index.html by hand.
const critical = esbuild.transformSync(fs.readFileSync(new URL('./css/critical.css', import.meta.url), 'utf8'),
  { loader: 'css', minify: true }).code.trim();
// …and MINIFY the sheet itself into the file index.html actually loads. css/app.css is the
// hand-written source (its /* @tokens-only */ regions are what test-theme.mjs reads); the page
// gets css/app.min.css, which is 26 kB → 14 kB gzipped on a render-blocking request.
const appCssMin = esbuild.transformSync(fs.readFileSync(new URL('./css/app.css', import.meta.url), 'utf8'),
  { loader: 'css', minify: true }).code;
fs.writeFileSync(new URL('./css/app.min.css', import.meta.url),
  '/* GENERATED by build.mjs from css/app.css — do not edit. */' + String.fromCharCode(10) + appCssMin + String.fromCharCode(10));
const CRIT = /(<!-- critical:start[^>]*-->)[\s\S]*?(<!-- critical:end -->)/;
if (!CRIT.test(idx)) {
  console.error('build.mjs: index.html has no <!-- critical:start --> … <!-- critical:end --> block.');
  process.exit(1);
}
idx = idx.replace(CRIT, (m, a, b) => a + '\n<style>' + critical + '</style>\n' + b);
console.log('css/critical.css: ' + (critical.length / 1024).toFixed(1) + ' kB inlined into index.html');
