...
-- ROLLBACK — restores the four originals verbatim.
...
-- create policy ws_insert on public.work_sessions for insert to authenticated with check (person = coalesce(auth.jwt() ->> 'name', person));
-- create policy ws_update on public.work_sessions for update to authenticated using (person = coalesce(auth.jwt() ->> 'name', person)) with check (person = coalesce(auth.jwt() ->> 'name', person));
